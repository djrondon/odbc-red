...
```Red
connection: odbc-open "dsn=<DATASOURCE>"
```
...
```Red
customers:  odbc-first connection
products:   odbc-first connection
orders:     odbc-first connection
```
...
The following examples should give an (informal) idea on how SQL statements are
executed. Here's a simple `SELECT` statement:

```Red
odbc-insert statement "select * from Film"
...
```Red
odbc-insert statement ["select * from Film where ID = ?" 6]
== [ID Category Description Length PlayingNow Rating TicketsSold Title]
odbc-copy statement
...
```Red
odbc-insert statement ["insert into Person (Name, Age) values (?, ?)" name age]
== 1
```
...
```Red
odbc-insert statement ["update Person set Name = ? where ID = ?" "nobody" 0]
== 0
```
...
```Red
odbc-insert statement ["delete from Subscription where SubscriptionID = ?" cancel]
== 5
```

## Affected rows
With row-changing `INSERT`, `UPDATE` and `DELETE` statements, `odbc-insert`
simply returns the number of rows affected by the statement:
...
With SELECT statements, `odbc-insert` returns a block of column names as Red
words (see below), on `odbc-copy` you retrieve the actual rows:

```Red
odbc-insert statement ["select LastName, FirstName from Person"]
== [LastName FirstName]
odbc-copy statement
== [
    ["Acre" "Anton"]
...
When you have to work with large result sets, you may want to retrieve results
in portions of `n` rows a time using refined `odbc-copy/part`:
...
```Red
columns: odbc-insert statement ["select ID, Category, Title from Film"]
== [ID Category Title]
...
```Red
columns: odbc-insert statement ["select ID, Descriptions, Title, Length, Category from Film"]
...
odbc-insert statement [products  1] ;-- preparation and execution
odbc-insert statement [products  2] ;-- execution only
odbc-insert statement [customers 3] ;-- preparation and execution
odbc-insert statement [customers 4] ;-- execution only
odbc-insert statement [products  1] ;-- again, preparation and execution
```
...
```Red
odbc-insert statement ["select ? from Table where ID = ?" "Test" 2]
odbc-copy statement
== [["Test"]]
```
...
```Red
odbc-insert statement ["select Weight from Person limit 1"]
type? first record: first records: odbc-copy statement
== float!
odbc-insert statement ["select cast (Weight as integer) from Person limit 1"]
type? first record: first records: odbc-copy statement
== integer!
```
...
```Red
odbc-insert statement ["select ? from Person limit 1" 1]
type? first record: first records: odbc-copy statement
== string!
odbc-insert statement ["select cast(? as integer) from Person limit 1" 1]
type? first record: first records: odbc-copy statement
== integer!
```
